...
#L1
w1 = tf.compat.v1.get_variable('w1',shape = [3,3,1,32]) # 초기값을 넣을 필요가없다
...
b1 = tf.compat.v1.Variable(tf.random.normal([64]),name = 'b1')
L5 = tf.compat.v1.matmul(L_flat, w5) +b1
...
b2 = tf.Variable(tf.random.normal([32]),name = 'b2')
...
loss = tf.reduce_mean(-tf.reduce_min(y + tf.math.log(hypothesis),axis=1)) # 카테고리컬
